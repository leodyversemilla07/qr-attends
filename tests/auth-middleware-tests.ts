...
async function createValidJWT(): Promise<string> {
  ...
  const secretRaw = Deno.env.get("JWT_SECRET") || "dev-secret-change-in-production";
...
Deno.test("Authentication Middleware - Valid JWT Token", async (t) => {
  await t.step("should set user in context when valid JWT provided", async () => {
    ...
    const result = await authMiddleware(mockContext as unknown as Parameters<typeof authMiddleware>[0]);
...
Deno.test("Require Auth Middleware", async (t) => {
  await t.step("should allow request when user is authenticated", async () => {
    ...
    const result = await requireAuth(mockContext as unknown as Parameters<typeof requireAuth>[0]);
    assert(result instanceof Response);
    assertEquals(await result.text(), "Protected content");
  });

  await t.step("should redirect to auth page when user not authenticated", async () => {
    const mockContext = {
      req: new Request("http://localhost/protected"),
      state: {},
      redirect: (url: string) => {
...
Deno.test("Require Admin Middleware", async (t) => {
  await t.step("should allow admin user access", async () => {
    ...
    const result = await requireAdmin(mockContext as unknown as Parameters<typeof requireAdmin>[0]);
    assert(result instanceof Response);
  ...
  await t.step("should deny access for regular user", async () => {
    ...
    assertEquals(await result.text(), "Forbidden: Admin or Officer access required");
  });
